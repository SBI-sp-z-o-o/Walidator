...
namespace ARLocation
{
    ...
    public class Location
    {
        ...
        {
            Console.WriteLine($"AAAAA ARLocationRoot is null: {arLocationRoot == null}");
            Console.WriteLine($"AAAAA User Unity Position: {userPosition}");
            Console.WriteLine($"AAAAA User GPS Location: {userLocation}");
            Console.WriteLine($"AAAAA Object GPS Location: {objectLocation}");
            Console.WriteLine($"AAAAA Height Is Relative: {heightIsRelative}");
            Console.WriteLine($"AAAAA Object Location IgnoreAltitude: {objectLocation.IgnoreAltitude}");

            var displacementVector = VectorFromTo(userLocation, objectLocation, objectLocation.IgnoreAltitude || heightIsRelative)
                .toVector3();
            Console.WriteLine($"AAAAA DisplacementVector: {displacementVector}");

            var displacementPosition = displacementVector;
            Console.WriteLine($"AAAAA DisplacementPosition: {displacementPosition}");

            var pos = userPosition + displacementPosition + new Vector3(0, (heightIsRelative && !objectLocation.IgnoreAltitude) ? ((float)objectLocation.Altitude - userPosition.y) : 0, 0);

            Console.WriteLine($"AAAAA Object Unity Position: {pos}");
